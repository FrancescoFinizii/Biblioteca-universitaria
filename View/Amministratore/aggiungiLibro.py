...
class AggiungiLibro(QDialog):


    def __init__(self):
        super(AggiungiLibro, self).__init__()
        loadUi("View/Amministratore/UI files/aggiungi-modifica libro.ui", self)
        self.confermaButton.clicked.connect(self.aggiungiLibro)
        self.annullaButton.clicked.connect(self.close)


    def aggiungiLibro(self):
        if self.notEmpty(self.titoloInput) and self.notEmpty(self.autoreInput) and self.notEmpty(self.editoreInput) and self.notEmpty(self.ISBNInput) and self.notEmpty(self.annoPubblicazioneInput) and self.notEmpty(self.genereInput):
                ...
                if all(x.isalpha() or x.isspace() for x in self.editoreInput.text()):
                    if self.annoPubblicazioneInput.text().isdigit():
                        if all(x.isalpha() or x.isspace() for x in self.genereInput.text()):
                            if all(x.isdigit() or x == "x" or x == "X" for x in self.ISBNInput.text()):
                                if 10 <= len(self.ISBNInput.text()) <= 13:
                                    if GestioneLibri.getDocumento(self.ISBNInput.text()) == None:
                                        GestioneLibri.aggiungiDocumento(titolo=self.titoloInput.text(),
                                        ...
                                        msgBox.setText("Aggiunta documento fallita\nIl codice ISBN è associato a un'altro documento")
                                        msgBox.exec_()
                                else:
                                    self.errorLabel.setText("il codice ISBN deve contenere tra le 10 e le 13 cifre")
                            else:
                                self.errorLabel.setText("ISBN non valido")
                        else:
